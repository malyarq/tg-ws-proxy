...
## Если окно настроек не открывается

При ошибке запуска интерфейса сообщение указывает путь к журналу. Откройте его через **«Открыть логи»** в трее и найдите `CTk root initialization failed` с текстом исключения либо сообщение о таймауте.

Ошибка Tcl/Tk (например, `Can't find a usable init.tcl`) не означает, что отсутствует `customtkinter`. В готовом `.exe` библиотеки уже включены в сборку; установка через системный `pip` не меняет их. Перед публикацией журнала удалите из него secret, ссылки подключения и личные пути.
